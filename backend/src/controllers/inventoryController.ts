...
export async function createInventoryItem(req: AuthRequest, res: Response) {
  try {
    ...
    const result = await executeQuery<any>(
      ...
      [effectiveBrandId, stockpoint_id || null, item_type, item_code, item_name, uom || 'pcs', current_stock || 0, minimum_stock, maximum_stock]
    )
    ...
    if ((current_stock || 0) > 0) {
      await executeQuery(
        ...
        [effectiveBrandId, itemId, 'opening_stock', current_stock, 'Initial stock', req.user!.id, 0, current_stock]
      )
    }
