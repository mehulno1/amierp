...
export async function generatePI(params: GeneratePIParams): Promise<void> {
  ...
  const gst_amount = (basic_total * gst_percent) / 100
  ...
  const result = await executeQuery<any>(
    `INSERT INTO proforma_invoices (brand_id, pi_no, order_id, pi_date, po_no, po_date, gst_percent, basic_total, gst_amount, total_amount, amount_in_words, bank_name, account_name, account_no, ifsc_code, swift_code, created_by)
     VALUES (?,?,?,CURDATE(),?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [brandId, pi_no, orderId, po_no || null, po_date || null, gst_percent, basic_total, gst_amount, total_amount, amount_in_words,
     brandData?.bank_name, brandData?.account_name, brandData?.account_no, brandData?.ifsc_code, brandData?.swift_code, userId]
  )
...
export async function getProformaInvoices(req: AuthRequest, res: Response) {
  try {
    ...
    const pis = await executeQuery(
      `SELECT pi.*, o.order_id as order_ref, c.name as client_name, b.name as brand_name FROM proforma_invoices pi
